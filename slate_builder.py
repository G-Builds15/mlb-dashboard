...
def write_slate(game_objects, pitcher_cards_by_game, best_bets):
        ...
        for i, go in enumerate(game_objects):
            if ('"' + key + '"') in go[:60]:
                if cards:
                    ...
                    go = re.sub(
                        r'pitcher:\{intro:"Props auto-graded from MLB Stats API[^"]*",cards:\[\]\}',
                        'pitcher:{' + f'intro:"Props graded {today}",cards:{cards_js}' + '}',
                        go
                    )
                    game_objects[i] = go
                    print(f"  ✓ {key}: {len(cards)} prop cards injected")
                elif not cards:
                    print(f"  ~ {key}: no prop cards graded")
                break
    ...
    output = f"""// SLATE DATA — update daily
// slate_data.js — {today}
